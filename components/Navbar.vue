  ...
  <nav id="header" class="w-full z-30 top-0 py-1">
    ...
    >
      ...
      <div class="order-2 md:order-3 flex items-center" id="nav-content">
        <a v-if="this.$auth.loggedIn" class="inline-block no-underline hover:text-black" @click="$router.push('/auth/profile')">
          <svg
        ...
        <a v-else class="inline-block no-underline hover:text-black" @click="$router.push('/auth/login')">
          <svg
            class="fill-current hover:text-black"
            xmlns="http://www.w3.org/2000/svg"
            width="24"
        ...
        <a class="pl-3 relative inline-block no-underline hover:text-black" 
        ...
        >
          ...
          <span v-if="countCart > 0" class=" absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full">{{ countCart }}</span>
        </a>
      </div>
    </div>
  </nav>
